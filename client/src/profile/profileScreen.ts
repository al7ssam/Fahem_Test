...
export async function renderProfileView(app: HTMLElement, opts: { onBack: () => void }): Promise<void> {
    ...
    <div class="app-screen min-h-screen text-white flex flex-col items-center justify-start p-4">
      <div class="max-w-lg w-full space-y-4 pt-4">
        <div class="flex items-center justify-between gap-2">
          ...
          <h1 class="text-xl font-bold text-right flex-1">الملف الشخصي</h1>
        </div>
        <p id="profile-load-msg" class="text-center text-slate-400 text-sm">جاري التحميل…</p>
  ...
  const msg = app.querySelector<HTMLParagraphElement>("#profile-load-msg")!;

  const res = await apiFetch("/api/profile/me");
  if (!res.ok) {
    msg.textContent = "تعذّر تحميل الملف الشخصي.";
    return;
  }
  const body = (await res.json()) as { ok?: boolean; profile?: ProfilePayload };
  if (!body.ok || !body.profile) {
    msg.textContent = "استجابة غير صالحة من الخادم.";
    return;
  }
  ...
  slot.innerHTML = `
    <div class="app-card p-6 space-y-5 text-right">
  ...
  function mountPicker(): void {
    pickerRoot.innerHTML = `
      <div class="country-picker-wrap">
        ...
        <div id="country-picker-panel" class="${pickerOpen ? "" : "hidden"} absolute z-50 mt-1 w-full rounded-xl border border-white/10 bg-slate-900 shadow-xl p-2 left-0 right-0">
